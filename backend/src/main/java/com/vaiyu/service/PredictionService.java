...
public class PredictionService {
    ...
    /**
     * The most recent stored run for a storm, or empty when none has been made.
     *
     * <p>The storm itself is looked up first so that the caller can tell
     * "there is no such storm" from "nobody has run the models on this storm
     * yet" — the second is the normal state of almost every storm in the
     * archive, and reporting it as a missing resource would make an ordinary
     * empty screen look like a failure.
     */
    @Transactional(readOnly = true)
    public Optional<PredictionRunDto> latest(UUID cycloneId) {
        if (!cyclones.existsById(cycloneId)) {
            throw new ResourceNotFoundException("No cyclone with id " + cycloneId);
        }
