...
| **Training table (tracks)** | `data/processed/observations.csv` (9.0 MB) | Present locally. 109,009 synoptic fixes, 4,305 storms, 1980-01-01 → 2026-09-08. Storms per basin: WP 1318, EP 957, SI 775, NA 711, SP 463, **NI 218** (Bay of Bengal 156, Arabian Sea 84), SA 1. Rebuilt after two filters were relaxed: a central pressure is no longer required on every fix (it had cost 18,472 fixes and 476 storms, worst in the North Indian Ocean, where only two thirds of wind-bearing fixes report one), and uncoded `NR` fixes are kept inside storms coded `TS` elsewhere, which is what the live adapter already did. Now carries a `sub_basin` column. |
